...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=416, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        print('RAJA start of ctr LoadImagesAndLabels')
        print('RAJA path = %s' %(path))
        print('RAJA img_size = %s' %(img_size))
        print('RAJA batch_size = %s' %(batch_size))
        print('RAJA augment = %s' %(augment))
        print('RAJA hyp = %s' %(hyp))
        print('RAJA rect. = %s' %(rect))
        print('RAJA image_weights = %s' %(image_weights))
        # Define labels
        ...
        self.imgs = [None] * n
        self.labels = [None] * n
        print('RAJA Going to enter if condition')
        if cache_labels or image_weights:  # cache labels for faster training
            print('RAJA Entered if condition')
            self.labels = [np.zeros((0, 5))] * n
            extract_bounding_boxes = False
            create_datasubset = False
            print('RAJA label_files      = %s' %((self.label_files)))
            print('RAJA label_files.type = %s' %(type(self.label_files)))
            #print('RAJA label_files.shape = %s' %(shape(self.label_files)))
            pbar = tqdm(self.label_files, desc='Caching labels')
            nm, nf, ne, ns, nd = 0, 0, 0, 0, 0  # number missing, found, empty, datasubset, duplicate
            print('RAJA Going to start for loop')
            for i, file in enumerate(pbar):
                ...
                if l.shape[0]:
                    #print('RAJA label.shape 0=%s 1=%s' %(l.shape[0],l.shape[1]))
                    assert l.shape[1] == 5, '> 5 label columns: %s' % file
                    #print('RAJA label = %s' %(l))
                    assert (l >= 0).all(), 'negative labels: %s' % file
                    #print('RAJA label dimension = %s' %(l[:, 1:]))
