...
class BilingualDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        enc_num_padding_tokens = self.seq_len - len(enc_input_tokens) - 2 # We will add only <s> and </s>
        # We will only add <s> and </s> only on the label
        dec_num_padding_tokens = self.seq_len - len(dec_input_tokens) - 1 # For encoding, we PAD both SOS and EOS. For decoding, we only pad SOS.   #THe model is required to predict EOS and stop on its own.
